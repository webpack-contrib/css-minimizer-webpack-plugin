...
declare class CssMinimizerPlugin<T = CssNanoOptionsExtended> {
     ...
     */
    constructor(options?: (BasePluginOptions & DefinedDefaultMinimizerAndOptions<T>) | undefined);
    /**
     * @private
...
declare namespace CssMinimizerPlugin {
    export { cssnanoMinify, cssoMinify, cleanCssMinify, esbuildMinify, parcelCssMinify, lightningCssMinify, swcMinify, Schema, Compiler, Compilation, WebpackError, JestWorker, RawSourceMap, Asset, ProcessOptions, Syntax, Parser, Stringifier, CssNanoOptions, Warning, WarningObject, ErrorObject, MinimizedResult, Input, CustomOptions, InferDefaultType, BasicMinimizerImplementation, MinimizerImplementation, MinimizerOptions, InternalOptions, InternalResult, Parallel, Rule, Rules, WarningsFilter, BasePluginOptions, MinimizerWorker, ProcessOptionsExtender, CssNanoOptionsExtended, DefinedDefaultMinimizerAndOptions, InternalPluginOptions };
}
...
type BasicMinimizerImplementation<T> = (input: Input, sourceMap: RawSourceMap | undefined, minifyOptions: InferDefaultType<T>) => Promise<MinimizedResult>;
type MinimizerImplementation<T> = T extends any[] ? T extends infer T_1 extends any[] ? { [P in keyof T_1]: BasicMinimizerImplementation<T[P]>; } : never : BasicMinimizerImplementation<T>;
type MinimizerOptions<T> = T extends any[] ? T extends infer T_1 extends any[] ? { [P in keyof T_1]?: InferDefaultType<T[P]> | undefined; } : never : InferDefaultType<T>;
